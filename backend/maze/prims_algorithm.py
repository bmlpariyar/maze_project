...
# def prims_algorithm(width, height,dead_end_probability=0.3):
...
#     # Add false openings and dead ends
#     for y, x in solution_path:
#         if random.random() < dead_end_probability:
#             open_neighbors = []
#             for dy, dx in directions:
#                 ny, nx = y + dy, x + dx
#                 if 1 <= ny < height - 1 and 1 <= nx < width - 1 and maze[ny][nx] == 1:
#                     open_neighbors.append((ny, nx))
            
#             if open_neighbors:
#                 wall_y, wall_x = random.choice(open_neighbors)
#                 maze[wall_y][wall_x] = 0
#                 maze[(y + wall_y) // 2][(x + wall_x) // 2] = 0
...
def prims_algorithm(width, height, dead_end_count=5):
    width = width if width % 2 == 1 else width + 1
    height = height if height % 2 == 1 else height + 1
